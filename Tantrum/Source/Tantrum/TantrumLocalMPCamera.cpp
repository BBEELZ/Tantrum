...
ATantrumLocalMPCamera::ATantrumLocalMPCamera(const FObjectInitializer& ObjectInitializer)
...
{
	Super::BeginPlay();
	ensureMsgf(GetWorld(), TEXT("ATantrumLocalMPCamera::BeginPlay() Missing World!"));
	TantrumGameModeBase = Cast<ATantrumGameModeBase>(GetWorld()->GetAuthGameMode());
}

void ATantrumLocalMPCamera::Tick(float DeltaTime)
{
